                ...
                <article key={run.id} className="rounded-3xl border border-white/8 bg-[#111114] p-6">
                  ...
                  <div className="mt-6">
                    <div className="rounded-2xl border border-white/8 bg-black/20 p-5">
                      <div className="flex items-center justify-between"><h4 className="text-sm font-medium">Media and evidence</h4><span className="text-xs text-zinc-600">{run.assets.filter((asset) => asset.status === "READY").length} ready files</span></div>
                      {run.assets.filter((asset) => asset.status === "READY").length === 0 ? <p className="mt-5 text-sm text-zinc-600">No reviewable media is available yet. Failed technical attempts stay out of the review workspace.</p> : <>
                        {run.assets.find((asset) => asset.type === "VIDEO" && asset.status === "READY") && <video className="mt-4 aspect-video w-full rounded-xl bg-black" controls preload="metadata" src={`/api/media/${run.assets.find((asset) => asset.type === "VIDEO" && asset.status === "READY")?.id}`}>{run.assets.find((asset) => asset.type === "CAPTIONS" && asset.status === "READY") && <track kind="captions" srcLang="en" label="English" default src={`/api/media/${run.assets.find((asset) => asset.type === "CAPTIONS" && asset.status === "READY")?.id}`} />}</video>}
                        <div className="mt-4 grid gap-2 sm:grid-cols-2">{run.assets.filter((asset) => asset.status === "READY").map((asset) => <div key={asset.id} className="flex items-center justify-between gap-3 rounded-xl bg-white/[0.03] px-3 py-2.5"><div className="min-w-0"><p className="text-xs font-medium capitalize">{label(asset.type)}</p><p className="mt-0.5 truncate text-[10px] text-zinc-600">{asset.storageKey}</p></div><div className="flex shrink-0 items-center gap-2"><Status value={asset.status} /><a className="text-xs text-violet-400 hover:text-violet-300" href={`/api/media/${asset.id}`} target="_blank" rel="noreferrer">Open ↗</a></div></div>)}</div>
                      </>}
                    </div>
                        ...
                        {(["X", "LINKEDIN"] as const).map((platform) => {
                          ...
                          return <section key={platform} className="rounded-2xl border border-white/8 bg-[#111114] p-5">
                            ...
                            {draft ? <>
                              ...
                              <SocialDraftApproval draftId={draft.id} approvedAt={draft.approvedAt} platform={platform} />
                              <SocialDraftPublish draftId={draft.id} status={draft.status} publishedUrl={draft.publishedPostUrl} platform={platform} />
