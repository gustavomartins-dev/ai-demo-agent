...
export function SocialDraftPublish({ draftId, status, publishedUrl, platform }: { draftId: string; status: string; publishedUrl: string | null; platform: "X" | "LINKEDIN" }) {
  ...
  return <form action={action} className="mt-4 rounded-xl border border-rose-400/15 bg-rose-400/[0.04] p-4">
    ...
    <p className="mt-1 text-xs leading-5 text-zinc-500">This sends the exact approved snapshot{platform === "LINKEDIN" ? " with the latest ready demo video attached" : ""} to the connected account. Double clicks and retries cannot create another attempt for the same approval.</p>
