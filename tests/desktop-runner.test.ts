...
describe("desktop Hermes runner", () => {
  ...
  it("retries a transient Computer Use outage only for an idempotent step", async () => {
    const root = path.join(os.tmpdir(), `desktop-runner-${crypto.randomUUID()}`);
    const project = path.join(root, "project");
    const output = path.join(root, "output");
    await mkdir(path.join(project, "bin"), { recursive: true });
    ...
    const retryPlan = {
      ...
      demo: {
        ...plan.demo,
        steps: [{ action: "fill" as const, target: { text: "Interval" }, value: "1", title: "Setting a short interval" }],
      },
    };
    const runHermes = vi.fn()
      .mockResolvedValueOnce({ stdout: JSON.stringify({ status: "failed", error: "Computer Use session became unavailable" }), stderr: "" })
      .mockResolvedValueOnce({ stdout: JSON.stringify({ status: "passed" }), stderr: "" });

    await runDesktopDemoWithReport(retryPlan, { projectPath: project, launchCommand: "./bin/product" },
      { command: "hermes", timeoutMs: 120_000 }, output, {
        allowedRoots: [root],
        launchApp: vi.fn().mockResolvedValue({ pid: 4321, stop: vi.fn().mockResolvedValue(undefined) }),
        resolveWindowId: async () => "0xe00004",
