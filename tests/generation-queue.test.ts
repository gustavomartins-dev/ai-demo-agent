...
describe("generation queue contract", () => {
  ...
  it("claims work through a locked, skip-locked PostgreSQL update", async () => {
    ...
    expect(source).toContain("candidate.\"status\" = 'RECORDING'");
    expect(source).toContain("candidate.\"status\" = 'DRAFTING'");
    expect(source).toContain('"workerId" IS NULL OR "leaseExpiresAt" <');
    expect(source).toContain("run.status === \"DRAFTING\" ? \"DRAFTING\"");
